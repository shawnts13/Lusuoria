 ...
 *
 * monthlyCap（当月封顶金额）只能设置在 maxCount 为空的那一档（即"最后一档、不封顶数量"）上，
 * 有上限（maxCount 非空）的档位不应该设置这个字段，由 Controller 保存时校验；但封的账不是
 * "这一档自己"的收入，而是"这个视频类型"当月的累计总收入（2026-08 修正，Shawn 反馈）——
 * 不管这个类型拆了几档、这一笔落在哪一档，只要这个类型当月累计挣到的钱到了这个金额，后面
 * 属于最后这一档的记录就不再计费；前面几档（非最后一档）的收入正常发放，不受这个封顶影响，
 * 只是同样会计入"累计"，让预算更快用完。见
 * CollaborationTrackingService.capAdjustedRate()。
 *
